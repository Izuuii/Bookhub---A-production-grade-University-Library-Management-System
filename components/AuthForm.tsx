...
const AuthForm = <T extends FieldValues>({
  ...
  const handleSubmit: SubmitHandler<T> = async (data) => {
    // optional custom logic before onSubmit
    await onSubmit(data);
  };
  ...
  return (
    <div className="flex flex-col gap-4">
      <h1 className="text-2xl font-semibold text-white">
        {isSignIn ? 'Welcome Back to BookHub ' : 'Create your library account '}
      </h1>
        ...
        >
          {Object.keys(defaultValues).map((field) => (
            <FormField
              ...
              name={field as keyof Path<T>}
              render={({ field }) => (
                <FormItem>
                  <FormLabel className="capitalized">
